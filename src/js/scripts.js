...
//func logout
  ...
  /**********************************************************************/
    ...
    function listPartnersFromLS(){
        ...
        <td><img id='img-displayed' src='" + item.partnerpic_src + "' class = 'id-"+ item.id +"'></td>\
  ...
  /**********************************************************************/
    //Edit Partner
  /**********************************************************************/
    //used to differentiate the id when adding vs editing
    var editing = false;

    var id;
    //listener to display the edit container and fill input fields with the clicked partner
    $(document.body).on('click', '#edit-partner-btn', function(){
        editing = true;
        ...
        oPartners.forEach(function (elem) {
            ...
            if(elem.id == id){
                ...
                $(".image-tag").attr('src', elem.partnerpic_src);
            }
        });
    });

    //used to save the path of the picture
    var picturePath;


    //saving the edited partner
    $(document.body).on('click', '#partner-save-btn', function () {
        ...
        obj.description = $("textarea[name='partner-descr']").val();
        obj.partnerpic_src = picturePath;
  ...
  /**********************************************************************/
    ...
    $('#add-partner-btn').on('click', function () {
        editing = false;
        $("#wdw-list-partners").hide();
        $(".image-tag").attr('src', "assets/data/images/noimage.png");
        $(".file-input").val("");
        $(".message").text("");
  ...
  /*Image Upload
   /**********************************************************************/
    ...
    var editCount = Math.floor((Math.random() * 100) + 1);
    $(document).ready(function (e) {
        $(".upload-image").on('submit',function(e) {
            ...
            //initialize new form data of the form to be send through ajax
            //append the id of the partner to be used as a file name in php
            var formData =  new FormData(this);
            //id for edit or add
            if(editing){
                partnerId = oPartners.length;
                editCount++;
                formData.append('filename', "edited-" + editCount + "-" + partnerId);
            } else {
                partnerId = oPartners.length + 1;
                formData.append('filename', partnerId);
            }
            ...
            $.ajax({
                ...
                // appended also the filename as the partner id
                data: formData,
                ...
                success: function(data)   // A function to be called if request succeeds
                {
                  ...
                  if(editing){
                      ...
                      //construct the path of the image based on the file type
                      if(data.indexOf("image/png") !== -1){
                          console.log("hi from 1");
                          picturePath = "assets/data/images/edited-" + editCount + "-" + partnerId + ".png";
                      } else
                      if(data.indexOf("image/jpg") !== -1){
                          console.log("hi from 2");
                          picturePath = "assets/data/images/edited-" + editCount + "-" + partnerId + ".jpg";
                      } else
                      if(data.indexOf("image/jpeg") !== -1){
                          console.log("hi from 3");
                          picturePath = "assets/data/images/edited-" + editCount + "-" + partnerId + ".jpeg";
                      }
                      ...
                      if(data.indexOf("image/png") !== -1){
                          console.log("hi from 1");
                          picturePath = "assets/data/images/" + partnerId + ".png";
                      } else
                      if(data.indexOf("image/jpg") !== -1){
                          console.log("hi from 2");
                          picturePath = "assets/data/images/" + partnerId + ".jpg";
                      } else
                      if(data.indexOf("image/jpeg") !== -1){
                          console.log("hi from 3");
                          picturePath = "assets/data/images/" + partnerId + ".jpeg";
                      }
                  }
                }
            });
